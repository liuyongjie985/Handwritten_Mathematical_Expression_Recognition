    ...
    for iter in range(len(labeledGraph) - 1):
        for jiter in range(iter, len(labeledGraph)):
            if (labeledGraph[jiter][jiter] != labeledGraph[iter][iter] and labeledGraph[jiter][jiter] != '-'):
                ...
                break;
            elif (jiter - 1 >= 0 and labeledGraph[jiter][jiter] == labeledGraph[iter][iter] and (
                        labeledGraph[jiter][jiter - 1] == '-')):
...
def SymbClassfier(UID, Symbols, Strokes, target2):
    for symb in Symbols.keys():
        ...
        for str in stroke_ids:
            ...
            writeSymbFeature(UID, symb_features, symb, target2)


def writeSymbFeature(UID, features, symb, target2):
    count = 0
    row = UID + ","
    count += 1
...
def pipeline_function(filename, target, target2):
    print("filename is %s" % filename)

    UID, Symbols, Strokes, GTgraph = read_inkml(filename)
    print("UID is %s" % UID)
    if Symbols != None:
        SymbClassfier(UID, Symbols, Strokes, target2)
    ...
    write_data(Normalized_features, UID, target)


def write_data(features, UID, target):
    row = ""
    count = 0
...
def main():
    if (len(sys.argv) < 2):
        ...
        target = open('data/features/SegmentorFeatures.csv', 'w')
        target2 = open('data/features/SymbolFeatures.csv', 'w')
        filename = sys.argv[1]
        read_files(filename, target, target2)
