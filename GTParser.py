...
def RDFTest(rfParser, SymbLabelList, NormalizedFeatures, pairs):
    ...
    probab = rfParser.predict_proba(NormalizedFeatures)
    allClasses = rfParser.classes_
...
def OR_fromat(outputdir, filename, LabelToSymbol, LabelToStroke, RelationGraph, SLT):
    UID = filename
    print(UID)
    ...
    for iter in range(len(RelationGraph)):
        for jiter in range(len(RelationGraph)):
            ...
            if (SLT[iter][jiter] == 1 and RelationGraph[iter][jiter] in labels):
                rel += RelationGraph[iter][iter] + ', ' + RelationGraph[jiter][jiter] + ', ' + RelationGraph[iter][
                    jiter] + ', ' + '1.0'
                target.write(rel + '\n')
